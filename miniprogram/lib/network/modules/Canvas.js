...
import { Hammer, selectiveDeepExtend } from "../../vis-util/esnext/esm/vis-util";

import WxCanvasHammer from "./WxCanvasHammer";
import WxCanvasHammerHandler from "./WxCanvasHammerHandler"
...
class Canvas {
   ...
   */
  constructor(body) {
    this._debugLog('Canvas construtor')
    ...
    this.options = {};
    this.defaultOptions = {
      autoResize: false, // hack true => false
      height: "100%",
      width: "100%",
    };
  ...
  _setCameraState() {
    if (
      ...
      this._debugLog('this.body.view.scale', this.body.view.scale, 'newScale', newScale)
   ...
   */
  _create(callback) {
    this._debugLog('Canvas _create')
    // remove all elements from the container element.
    if( this.body.container && this.body.container.hasChildNodes){ // hack
    ...
    if( document ){
      ...
      this.body.view.translation = {
        // x: 0.5 * this.frame.canvas.width,
        // y: 0.5 * this.frame.canvas.height,
        x: 0.5 * this.frame.canvas.width,
        y: 0.5 * this.frame.canvas.height,
      };
  ...
  _bindHammer() {
    ...
    // init hammer
    this.hammer = typeof wx !== 'undefined' ? new WxCanvasHammer() : new Hammer(this.frame.canvas);
    ...
    this.wxCanvasHammerHandler = new WxCanvasHammerHandler(this.hammer)
  }
  
  onWxCanvasEvent(event) {
    this.wxCanvasHammerHandler.onWxCanvasEvent(event)
  }
  ...
  setSize(width = this.options.width, height = this.options.height) {
    ...
    // this._debugLog('setSize', width, height)

    let emitEvent = false;
    const oldWidth = 0 // this.frame.canvas.width; // hack
    const oldHeight = 0 // this.frame.canvas.height;
    ...
    if (
      ...
      if( this.frame.canvas && this.frame.canvas.sytle){ // hack
      ...
      this.canvasViewCenter = {
        x: 0.5 * this.frame.canvas.clientWidth, // this.frame.clientWidth, // hack
        y: 0.5 * this.frame.canvas.clientHeight // this.frame.clientHeight,
      };
    ...
    if (emitEvent === true && this.frame.canvas) {
      this._debugLog('emit resize')
  ...
  _determinePixelRatio() {
    ...
    }else if( typeof wx !== 'undefined' ){// hack
      numerator = wx.getWindowInfo().pixelRatio
    }
  ...
  _XconvertDOMtoCanvas(x) {
    const cx = (x - this.body.view.translation.x) / this.body.view.scale;
    // this._debugLog('dom2canvas x', x, this.body.view.translation.x, 'cx', cx)
    return cx
  }
  ...
  _YconvertDOMtoCanvas(y) {
    const cy = (y - this.body.view.translation.y) / this.body.view.scale;
    // this._debugLog('dom2canvas y', y, this.body.view.translation.y, 'cy', cy)
    return cy
  }
