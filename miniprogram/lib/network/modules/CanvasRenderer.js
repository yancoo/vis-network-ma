import { selectiveDeepExtend } from "../../vis-util/esnext/esm/vis-util";
...
class CanvasRenderer {
  ...
  constructor(body, canvas) {
    ...
    this.body = body;
    this.canvas = canvas;
    console.log('CanvasRenderer', this.canvas.frame)
  ...
  _requestNextFrame(callback, delay) {
    ...
    if( typeof window === "undefined" && typeof wx !== 'undefined' ){
      window = {
        requestAnimationFrame: this.canvas.frame.canvas.requestAnimationFrame,
        clearTimeout: (timeoutId) => {
          clearTimeout(timeoutId)
        },
        setTimeout: (callback, ...args)=>{
          return setTimeout(callback, ...args)
        }
      }
    }
  ...
  _determineBrowserMethod() {
    if (typeof window !== "undefined") {
      navigator = navigator || {userAgent: ''} // hack
